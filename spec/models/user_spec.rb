...
RSpec.describe User, type: :model do
  ...
  context "can not be created without validations on " do
    it "first_name exist" do
      user = User.new(:last_name => "Taman", :email => "abdelrhmanTaman@gmail.com", :password => "ghgh22", :phone => "01223765378", :gender => "male")
      expect(user.valid?).to eq(false)
    end

    it "last_name exist" do
      user = User.new(:first_name => "abdelrhman", :email => "abdelrhmanTaman@gmail.com", :password => "ghgh22", :phone => "01223765378", :gender => "male")
      expect(user.valid?).to eq(false)
    end

    it "email exist" do
      user = User.new(:first_name => "abdelrhman", :last_name => "Taman", :password => "ghgh22", :phone => "01223765378", :gender => "male")
      expect(user.valid?).to eq(false)
    end

    it "can not create 2 users with same email" do
      user1 = User.create(:first_name => "abdelrhman", :last_name => "Taman", :email => "abdelrhmanTaman@gmail.com", :password => "ghgh22", :phone => "01223765378", :gender => "male")
      user2 = User.create(:first_name => "lina", :last_name => "elshrife", :email => "abdelrhmanTaman@gmail.com", :password => "ghhhh", :phone => "01277765378", :gender => "female")
      expect(user1.valid?).to eq(true)
      expect(user2.valid?).to eq(false)
    end

    it "password exist" do
      user = User.new(:first_name => "abdelrhman", :last_name => "Taman", :email => "abdelrhmanTaman@gmail.com", :phone => "01223765378", :gender => "male")
      expect(user.valid?).to eq(false)
    end

    it "can not create 2 users with same password" do
      user1 = User.create(:first_name => "abdelrhman", :last_name => "Taman", :email => "abdelrhmanTaman@gmail.com", :password => "ghgh22", :phone => "01223765378", :gender => "male")
      user2 = User.create(:first_name => "lina", :last_name => "elshrife", :email => "linaElshrif@gmail.com", :password => "ghgh22", :phone => "01277765378", :gender => "female")
      expect(user1.valid?).to eq(true)
      expect(user2.valid?).to eq(false)
    end

    it "phone exist" do
      user = User.new(:first_name => "abdelrhman", :last_name => "Taman", :email => "abdelrhmanTaman@gmail.com", :password => "ghgh22", :gender => "male")
      expect(user.valid?).to eq(false)
    end

    it "can not create 2 users with same phone" do
      user1 = User.create(:first_name => "abdelrhman", :last_name => "Taman", :email => "abdelrhmanTaman@gmail.com", :password => "ghgh22", :phone => "01223765378", :gender => "male")
      user2 = User.create(:first_name => "lina", :last_name => "elshrife", :email => "linaElshrif@gmail.com", :password => "ghhhh", :phone => "01223765378", :gender => "female")
      expect(user1.valid?).to eq(true)
      expect(user2.valid?).to eq(false)
    end
    ...
    it "specific format for email" do
      user = User.new(:first_name => "abdelrhman", :last_name => "Taman", :email => "abdelrhmanTamangmail.com", :password => "ghgh22", :phone => "01223765378", :gender => "male")
      expect(user.valid?).to eq(false)
    end
  end
